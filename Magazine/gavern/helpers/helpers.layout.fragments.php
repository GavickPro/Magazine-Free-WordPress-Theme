...
function gavern_menu($menuname, $fullname, $params = null) {			
	...
	if(gk_show_menu($menuname)) {
		...
		wp_nav_menu(array(
			  ...
			  'container_class' => isset($container_class) ? $container_class : 'menu-container', 
			  'container_id'    => $fullname,
			  'menu_class'      => 'menu ' . $menu_class . ' ' . $tpl->menu[$menuname]['style'], 
...
function gavern_comment_template( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>">	
			<aside>
				<?php echo get_avatar( $comment, ($comment->comment_parent == '0') ? 72 : 72); ?>
			</aside>
...
?>
		<?php if($param_aside) : ?>
	 	<ul>
	 		...
	 		<li class="format gk-format-<?php echo get_post_format(); ?>">
	 		...
	 		<?php if(!(is_tag() || is_archive() || is_home() || is_search())) : ?>
		 		...
		 		<li class="comments">		 			
		 			<?php 
		 				comments_popup_link(
		 					'<span class="leave-reply">' . __( 'Leave a comment', GKTPLNAME ) . '</span>', 
		 		...
		 		<?php echo edit_post_link(__( 'Edit', GKTPLNAME ), '<li class="edit">', '</li>'); ?>
	 		<?php endif; ?>
 		</ul>
 		<?php endif; ?>
 	<?php
